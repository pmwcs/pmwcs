...
describe('Badge', () => {
  ...
  it('align', () => {
    ...
    expect(el.html().includes('pmwc-badge--align-end')).toBe(true)

    const el2 = mount(<Badge align='start' />)
    expect(el2.html().includes('pmwc-badge--align-start')).toBe(true)
  })

  it('exited', () => {
    const el = mount(<Badge exited />)
    expect(el.html().includes('pmwc-badge--exited')).toBe(true)
  })
