...
describe('Radio', () => {
  ...
  test('can be checked', () => {
    const radio = mount(<Radio checked onChange={() => {}} />)
    expect(
      (radio.find('input').getDOMNode()).checked
    ).toEqual(true)
  })

  test('handles onChange', () => {
    let value = 0
    const checkbox = mount(<Radio checked onChange={() => value++} />)
  ...
  test('can be disabled', () => {
    ...
    expect(
      (radio.find('input').getDOMNode()).disabled
    ).toEqual(true)
  })
